...
export function Callout({
  ...
  const Icon = s.icon;
  return (
    <div className={cn("flex gap-3 rounded-xl border p-4", s.border, s.bg)}>
      <Icon className={cn("mt-0.5 size-4 shrink-0", s.color)} />
      <div>
        {title && <div className="mb-0.5 text-sm font-medium text-bright">{title}</div>}
        <p className="text-sm leading-relaxed text-dimmed">{text}</p>
      </div>
    </div>
  );
}
...
export function Steps({ steps }: { steps: { title: string; text: string }[] }) {
  return (
    <div className="rounded-[20px] border border-grid-dimmed bg-charcoal-850 p-6">
      <ol className="space-y-4">
        {steps.map((s, i) => (
          <li key={i} className="flex gap-3">
            ...
            <div>
              <div className="text-sm font-medium text-bright">{s.title}</div>
              <p className="mt-0.5 text-sm leading-relaxed text-dimmed">{s.text}</p>
            </div>
          </li>
        ))}
      </ol>
      ...
      <dl className="space-y-3">
        {terms.map((t, i) => (
          <div key={i} className="border-b border-grid-bright pb-3 last:border-0 last:pb-0">
            ...
            <dd className="mt-0.5 text-sm leading-relaxed text-dimmed">{t.definition}</dd>
          </div>
        ))}
      </dl>
    </div>
...
export function Compare({
  ...
  return (
    <div className="overflow-hidden rounded-[20px] border border-grid-dimmed bg-charcoal-850">
      {title && (
        ...
        {[a, b].map((col, ci) => (
          <div key={ci} className="p-5">
            ...
            <ul className="space-y-2">
              {col.points.map((p, i) => (
                <li key={i} className="flex gap-2 text-sm leading-relaxed text-dimmed">
