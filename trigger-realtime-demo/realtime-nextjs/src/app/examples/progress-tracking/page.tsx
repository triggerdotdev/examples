...
// Display code shown in CodePanel (line numbers in code-mappings.ts reference these)
const rawCodeFiles = [
  {
    name: "task.ts",
    language: "typescript",
    code: `// Import task and our typed stream
...
export const processDataTask = task({
  ...
  run: async (payload: { items: number }) => {
    ...
    for (let i = 0; i < total; i++) {
      ...
      // Stream progress to frontend (received instantly)
...
// Create typed stream - frontend subscribes to this
...
export async function startProcessing(itemCount: number) {
  ...
  // Create token scoped to only this run
