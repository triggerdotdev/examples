...
async function testSupabase() {
  ...
  console.log('SUPABASE_URL:', url ? `✅ ${url.substring(0, 40)}...` : '❌ NOT SET');
  console.log('SUPABASE_PRIVATE_KEY:', privateKey ? `✅ ${privateKey.substring(0, 20)}...` : '❌ NOT SET');
  console.log('');
  ...
  testChannel.subscribe((status, err) => {
    ...
    if (status === 'SUBSCRIBED') {
      ...
      testChannel.send({
        ...
        console.log('🎉 All tests passed! Your Supabase configuration is correct.\n');
        process.exit(0);
      }).catch((error) => {
    ...
    if (status === 'TIMED_OUT') {
      ...
      console.log('2. Try using the service role key from Supabase dashboard\n');
    }
